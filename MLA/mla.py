...
from ohara.embeddings_pos.rotatory import precompute_freqs_cis
from ohara.embeddings_pos.rotatory import apply_rope
...
class MultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.decompress_q_rope = self.num_heads * self.rope_head_dim

        # key and value compression
        self.compress_kv_linear = nn.Linear(self.dim, self.kv_lora_rank, bias=False)  # W_DKV
        self.decompress_k_nope = nn.Linear(self.kv_lora_rank, self.nope_di, bias=False)
...
def forward(self, x: Tensor, mask: torch.Tensor, freqs_cis: Tensor):
    ...
    q_rope = key_rope/self.num_heads
...
class Attention(nn.Module):
    def __init__(self, config: Config):
        ...
        self.num_kv_heads % self.num_kv_heads == 0
